...
L109 — Script-owned listing pages must not be manually edited
What happened: The filter-by-URL feature was added to product-listing.html and
th/product-listing.html as standalone files. Both files are generated by generate_products.py
and would have been overwritten on the next workflow run, losing the fix entirely.
Rule: Before editing any HTML file, check if it appears in the script-owned list in
PROJECT_MASTER.md. If it does, the fix belongs in the Python generator, not the HTML file.
Pattern: grep for the filename in generate_*.py before touching it manually.

L110 — URL query params are the cleanest way to pre-select filter state
What happened: Homepage category cards needed to deep-link to the product listing
with a specific filter pre-applied (e.g. click Reformer → listing shows only reformers).
Rule: Use ?filter=CategoryName in the link href. On the listing page, read
new URLSearchParams(window.location.search).get('filter') and programmatically
click the matching filter button. No cookies, no localStorage, no backend needed.
Pattern:
javascript// In listing page script — runs after filter buttons are defined
(function() {
...
Note: The filter value in the URL must exactly match the data-filter attribute value,
including case and spaces (use + for spaces in URL: ?filter=Wunda+chair).

L111 — Scroll-reveal animation breaks elements inside z-index overlay wrappers
What happened: FAQ questions were added to the IntersectionObserver scroll-reveal
animation (opacity: 0 → revealed). The FAQ section was then wrapped in a
position: relative; z-index: 1 div for the background image overlay. The observer
stopped detecting the elements correctly — questions stayed invisible permanently.
Rule: Do not put scroll-reveal animation on elements that live inside stacked
z-index overlay wrappers. The observer fires based on viewport intersection of the
element, which can behave unexpectedly inside complex stacking contexts.
Fix: Remove the animated element from the observer selector entirely. The background
image already provides visual interest — the fade-in is not needed and caused the bug.

L112 — Anchor tags need explicit color:inherit and text-decoration:none for card links
What happened: Category cards were converted from <div> to <a> tags to enable
deep-linking to the product listing. Browser default link styles (blue color, underline)
appeared on the card text even though the CSS targeted .category-card.
Rule: Any time a structural element is changed from <div> to <a>, immediately add
text-decoration: none; color: inherit; display: block; to its CSS rule. These three
properties together neutralise all browser default anchor styling.

L113 — aspect-ratio on background-image cards should control height not shape
What happened: Category cards used aspect-ratio: 3/4 (portrait). The images were
AI-generated landscape/square compositions — the portrait crop cut the subject badly
and made the cards too tall, pushing content below the fold on mobile.
Rule: For cards with background-image, use width: 100%; height: [fixed]px instead
of aspect-ratio. This fills the card width fully and only crops height — much more
controllable. Use background-position: center center to crop from the middle.
Use aspect-ratio only when you control the image dimensions exactly (e.g. video embeds).

L114 — Mobile cache for injector JS requires Cloudflare purge, not just incognito
What happened: A CSS fix was confirmed working on desktop but mobile still showed
the old design. Opening incognito on mobile did not help.
Rule: Mobile browsers do not have Cmd+Shift+R hard reload. When an injector JS file
changes, mobile will serve stale from Cloudflare edge cache regardless of incognito.
